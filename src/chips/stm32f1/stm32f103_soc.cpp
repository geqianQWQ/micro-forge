...
std::expected<std::unique_ptr<Stm32f103Soc>, std::string> Stm32f103Soc::create() {
    ...
    auto clock = sim::VirtualClock(
        std::span<const sim::DomainConfig>(stm32f103_default_clocks, kClockDomainCount));
    m.coord = std::make_unique<sim::SimulationCoordinator>(std::move(clock));
    m.coord->set_cpu(cm3->GetWeak());
    m.coord->add_tickable(p.systick.GetWeak(), domain_index(ClockDomain::Sysclk));
    m.coord->add_tickable(p.tim2.GetWeak(), domain_index(ClockDomain::Apb1));

    return soc;
}

std::expected<void, std::string> Stm32f103Soc::load_elf(std::span<const uint8_t> data) {
...
std::expected<void, std::string> Stm32f103Soc::load_bin(uint32_t base,
