...
Expected<data_t> Stm32f1Gpio::read(addr_t offset, Width w) {
    ...
    switch (offset) {
    case 0x00: return crl_;
    case 0x04: return crh_;
    case 0x08: return idr_;
    case 0x0C: return odr_;
    case 0x18: return lckr_;
...
Expected<void> Stm32f1Gpio::write(addr_t offset, data_t data, Width w) {
    ...
    switch (offset) {
    case 0x00: crl_  = data; return {};
    case 0x04: crh_  = data; return {};
    case 0x08: return std::unexpected(BusError::ReadOnly); // IDR read-only
    case 0x0C: {
        uint32_t old = odr_;
        odr_ = data;
...
void Stm32f1Gpio::configure_pin(uint8_t pin, periph::PinMode mode,
    ...
    case periph::PinMode::Output:
        switch (speed) {
        case periph::PinSpeed::Low:    cfg = 0x2; break; // Output push-pull, 2MHz
        case periph::PinSpeed::Medium: cfg = 0x5; break; // Output push-pull, 10MHz
        case periph::PinSpeed::High:   cfg = 0x3; break; // Output push-pull, 50MHz
        }
        break;
    case periph::PinMode::Alternate: cfg = 0x8; break; // AF push-pull
    case periph::PinMode::Analog:    cfg = 0x0; break;  // Analog same as input
    }
...
void Stm32f1Gpio::on_odr_changed(uint32_t old_odr, uint32_t new_odr) {
    ...
    for (uint8_t i = 0; i < 16; ++i) {
        if ((changed >> i) & 1) {
            ...
            fprintf(stderr, "[GPIO] GPIO%c.PIN%u -> %s\n",
                    port_id_, i, high ? "HIGH" : "LOW");
            if (on_pin_change_) on_pin_change_(i, high);
        }
    }
}
