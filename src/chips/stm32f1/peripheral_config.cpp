...
Expected<void> configure_peripherals(memory::Bus& bus, Stm32f1Rcc& rcc,
    ...
    result =
        bus.map(memory::region(0x4001'0800_addr, 0x400_addr, gpioa.GetWeak()));
    if (!result) {
        return result;
    }

    result =
        bus.map(memory::region(0x4001'0C00_addr, 0x400_addr, gpiob.GetWeak()));
    if (!result) {
        return result;
    }

    result =
        bus.map(memory::region(0x4001'1000_addr, 0x400_addr, gpioc.GetWeak()));
    if (!result) {
        return result;
    }

    result =
        bus.map(memory::region(0x4001'3800_addr, 0x400_addr, usart1.GetWeak()));
    if (!result) {
        return result;
    }

    result =
        bus.map(memory::region(0x4000'0000_addr, 0x400_addr, tim2.GetWeak()));
    if (!result) {
        return result;
    }
