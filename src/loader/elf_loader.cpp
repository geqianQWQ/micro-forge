...
template <typename T>
std::expected<T, std::string> read_struct(std::span<const uint8_t> data, size_t offset) {
    if (offset + sizeof(T) > data.size()) {
        return std::unexpected("ELF data truncated at offset " + std::to_string(offset));
    }
    T val;
...
std::expected<void, std::string> write_segment(
    memory::Bus& bus, uint32_t addr,
    ...
    for (size_t i = 0; i < seg_data.size(); i += 4) {
        ...
        if (!res.has_value()) {
            return std::unexpected("failed to write segment at 0x" +
                ([](uint32_t v) { char buf[16]; std::snprintf(buf, sizeof(buf), "%08X", v); return std::string(buf); })(addr + static_cast<uint32_t>(i)));
        }
    }
    ...
    for (size_t i = seg_data.size(); i < memsz; i += 4) {
        ...
        if (!res.has_value()) {
            return std::unexpected("failed to zero-fill BSS at offset " + std::to_string(i));
        }
    }
...
load_elf(memory::Bus& bus, std::span<const uint8_t> elf_data) {
    ...
    // Validate ELF header
    auto mut_span = std::span<uint8_t>(const_cast<uint8_t*>(elf_data.data()), elf_data.size());
    ...
    if (!ehdr) return std::unexpected(ehdr.error());

    static_assert(sizeof(Elf32_Ehdr) == 52);

    // Iterate program headers
    for (uint16_t i = 0; i < ehdr->e_phnum; ++i) {
        size_t ph_offset = ehdr->e_phoff + static_cast<size_t>(i) * ehdr->e_phentsize;
        auto phdr = read_struct<Elf32_Phdr>(elf_data, ph_offset);
        if (!phdr) return std::unexpected(phdr.error());
