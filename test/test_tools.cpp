...
protected:
    ...
    void SetUp() override {
        ASSERT_TRUE(bus.map(memory::region(0x0000, mem.size(), mem.GetWeak())).has_value());
    }
};
...
TEST_F(ToolsTest, MemoryDumpBasic) {
    ...
    memory_dump(bus, 0x0000, 8, [&](std::string_view line) {
...
TEST_F(ToolsTest, MemoryDumpMultipleLines) {
    ...
    std::vector<std::string> lines;
    memory_dump(bus, 0x0000, 32, [&](std::string_view line) {
        lines.emplace_back(line);
    });
...
TEST_F(ToolsTest, MemoryDumpUnmappedReadsFF) {
    ...
    std::vector<std::string> lines;
    memory_dump(bus, 0xF000, 16, [&](std::string_view line) {
        lines.emplace_back(line);
    });
...
TEST_F(ToolsTest, MemoryDumpPartialLine) {
    ...
    std::vector<std::string> lines;
    memory_dump(bus, 0x0000, 3, [&](std::string_view line) {
        lines.emplace_back(line);
    });
